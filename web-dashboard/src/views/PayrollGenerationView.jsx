                  ...
                  <div className="md:col-span-2 space-y-4">
                    ...
                    <div className="grid grid-cols-5 sm:grid-cols-7 gap-2.5 bg-slate-950/40 p-4 rounded-2xl border border-slate-700/40">
                      {Array.from({ length: slipData.daysInMonth }).map((_, index) => {
                        ...
                        const isPresent = attendance && ['APPROVED', 'CHECKED_IN', 'CHECKED_OUT'].includes(attendance.status);
                        
                        // Check overtime record
                        const otRecord = slipData.overtime.find(ot => ot.date.split('T')[0] === dateStr && ot.status === 'APPROVED');
                        const otHours = otRecord ? parseFloat(otRecord.overtime_hours) : 0;
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 hidden group-hover:block z-10 w-44 bg-slate-950 border border-slate-700 p-2.5 rounded-xl text-[10px] text-slate-300 shadow-2xl space-y-1">
                              ...
                              {isPresent ? (
                                <>
                                  <div>In: {attendance.check_in_time ? new Date(attendance.check_in_time).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : 'N/A'}</div>
                                  <div>Out: {attendance.check_out_time ? new Date(attendance.check_out_time).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : 'N/A'}</div>
                                  {otHours > 0 && <div className="text-amber-400 font-bold">Approved OT: {otHours} hrs</div>}
                                </>
                              ) : (
                                <div>No attendance record recorded.</div>
                              )}
                            </div>
                          </div>
                        );
